...
type RootStackParamList = {
  PerformanceMetrics: { 
    frontEyeMetrics: Metrics;
    frontMaxMetrics: Metrics;
    frontWaistMetrics: Metrics;
    sideEyeMetrics: Metrics;
    sideMaxMetrics: Metrics;
    sideWaistMetrics: Metrics;
    selectedPlayer: { name: string; image: string };
    overallComparisonScore: number;
    user: string;
  }
...
const PerformanceMetrics: React.FC<HomeProps> = ({ navigation }) => {
  ...
  // Circle configurations (unchanged)
  const radius = 45;
  const strokeWidth = 10;
  const circumference = 2 * Math.PI * radius;
  const offset = circumference * (1 - overallComparisonScore / 100);
  ...
  // Helper to render a metrics group table for a given group title and metrics object
  const renderMetricsGroup = (groupTitle: string, metrics: Metrics) => (
    <View style={styles.groupContainer}>
      ...
      <View style={styles.table}>
        ...
        {Object.entries(metrics).map(([metric, score], index) => (
          <View key={index} style={styles.row}>
            <Text style={styles.cell}>{metric}</Text>
            <Text style={styles.cell}>{score}</Text>
          </View>
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <Text style={styles.subTitle}>Front Metrics</Text>
        {renderMetricsGroup('Release Metrics', frontMaxMetrics)}
        {renderMetricsGroup('Eye Level Metrics', frontEyeMetrics)}
        {renderMetricsGroup('Waist Metrics', frontWaistMetrics)}

        {/* Side Metrics Section */}
        <Text style={styles.subTitle}>Side Metrics</Text>
        {renderMetricsGroup('Release Metrics', sideMaxMetrics)}
        {renderMetricsGroup('Eye Level Metrics', sideEyeMetrics)}
        {renderMetricsGroup('Waist Metrics', sideWaistMetrics)}

        <View style={styles.buttonContainer}>
          <Button title="Home" onPress={() => navigation.navigate('Home', { user })} />
          <Button title="Switch Players" onPress={() => navigation.navigate('Compare', { user })} />
          <Button title="Upload More Videos" onPress={() => navigation.navigate('UploadVideos', { selectedPlayer, user })} />
        </View>
      </View>
    </ScrollView>
  );
